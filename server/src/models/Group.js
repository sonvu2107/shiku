...
// NEW: Compound indexes for /api/groups/my-groups query optimization
// These eliminate COLLSCAN + SORT by covering filter + sort in index
groupSchema.index({ isActive: 1, owner: 1, updatedAt: -1 });
groupSchema.index({ isActive: 1, 'members.user': 1, updatedAt: -1 });
// For role-specific queries (role != 'all')
groupSchema.index({ isActive: 1, 'members.user': 1, 'members.role': 1, updatedAt: -1 });
