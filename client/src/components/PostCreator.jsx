...
const PostCreator = forwardRef(function PostCreator({ user, groupId = null, hideTrigger = false }, ref) {
  ...
  const [advancedMode, setAdvancedMode] = useState(false); // Compact (false) vs Advanced (true) editor mode
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate content is required (title is now optional, auto-generated by BE)
    ...
    // Validate content length for advanced mode
    if (advancedMode && trimmedContent.length > 5000) {
      setErr("Nội dung không được quá 5000 ký tự");
      showError("Nội dung không được quá 5000 ký tự");
      return;
    }
    ...
    try {
      ...
      // Build clean payload - only send title if in advanced mode and non-empty
      const payload = {
      ...
      // Only include title if in advanced mode and has value
      if (advancedMode && title.trim()) {
              ...
              <div className="overflow-y-auto max-h-[calc(90vh-120px)]">
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                      ...
                      <div className="relative">
                        <textarea
                          ...
                          onChange={(e) => {
                            ...
                            // Auto-switch to advanced mode if pasting > 500 chars
                          ...
                          rows={3}
                          maxLength={500}
                          className="w-full p-4 bg-neutral-50 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl text-[15px] leading-relaxed text-neutral-900 dark:text-white placeholder-neutral-400 dark:placeholder-neutral-500 focus:outline-none focus:ring-2 focus:ring-neutral-300 dark:focus:ring-neutral-700 resize-none transition-all"
                        />
                        <span className={`absolute bottom-3 right-3 text-xs transition-colors ${content.length >= 450
                          ? content.length >= 500
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="relative">
                        ...
                        <div className="flex justify-end mt-1">
                          <span className={`text-xs transition-colors ${content.length >= 4500
                            ? content.length >= 5000
                              ? 'text-red-500 font-semibold'
                              : 'text-amber-500'
                            : 'text-neutral-400'
                  ...
                  <div className="flex items-center gap-1 sm:gap-2 pt-3 sm:pt-4 border-t border-neutral-200 dark:border-neutral-800 flex-wrap">
                    {/* Media Upload - Available in both modes */}
                    ...
                    {/* Poll & YouTube - Only in Advanced mode */}
                    {advancedMode && (
                      <>
                        <button
                          type="button"
