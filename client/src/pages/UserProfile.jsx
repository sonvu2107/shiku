...
export default function UserProfile() {
  ...
  async function loadProfile() {
    try {
      ...
      // Map backend response to friendshipStatus
      let friendshipStatus = 'none';
      if (data.user.isFriend) {
        friendshipStatus = 'friends';
      } else if (data.user.hasPendingRequest) {
        friendshipStatus = data.user.pendingRequestDirection === 'sent' ? 'request_sent' : 'request_received';
      }
      
      console.log('Mapped friendshipStatus:', friendshipStatus);
  ...
  <div className="w-full px-6 py-6 pt-24">
      <div className="card max-w-2xl mx-auto space-y-6">
        ...
        <div className="flex items-start gap-4">
          <img
            src={user.avatarUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(user.name)}&background=cccccc&color=222222&size=128`}
            alt="avatar"
          ...
          <div className="flex-1">
            ...
            <p className="text-gray-600 mb-3">
              {profile.friendshipStatus === 'friends' ? 'Đã là bạn bè' : 'Chưa là bạn bè'}
            </p>
          ...
          <div className="space-y-3">
            {user.birthday && (
              <div className="flex items-center gap-3">
                ...
                <span>Sinh nhật: {new Date(user.birthday).toLocaleDateString('vi-VN')}</span>
              </div>
            )}
