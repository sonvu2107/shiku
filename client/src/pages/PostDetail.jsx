...
export default function PostDetail() {
  ...
  // Get display image: coverUrl first, then first image from content
      ...
      <div className="card max-w-4xl mx-auto">
        <div className="flex items-center gap-3 mb-1">
          <Link to={`/user/${p.author?._id}`}>
            <img
              src={p.author?.avatarUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(p.author?.name || "")}&background=cccccc&color=222222&size=64`}
              alt="avatar"
        ...
        <div className="text-sm text-gray-600 mb-3">
          <Link to={`/user/${p.author?._id}`} className="hover:text-blue-600 font-medium">
            {p.author?.name}
          </Link> • {new Date(p.createdAt).toLocaleString()}
        ...
        <div className="mt-4 flex items-center gap-2 flex-wrap">
          <button className="btn-outline flex items-center gap-2" type="button" onClick={() => setShowEmote(!showEmote)}>
            ...
            <span className="ml-2 flex gap-2">
              {Object.entries(counts).map(([emo, count]) =>
                count > 0 ? <span key={emo}>{emo} {count}</span> : null
              )}
            </span>
          </button>
          {showEmote && (
            <div className="flex gap-2 mt-2">
              {emotes.map(e => (
                <button key={e} className="btn-outline" type="button" onClick={() => emote(e)}>{e}</button>
              ))}
            </div>
          )}
          {user && (user._id === p.author?._id || user.role === "admin") && (
            <div className="flex gap-2">
              ...
              <button className="btn-outline text-red-600" type="button" onClick={deletePost}>Xóa bài</button>
            </div>
          )}
        </div>
      </div>
